...
function initGenres() {
    ...
    genreFilter.innerHTML = `
        <button class="genre-btn active" onclick="filterByGenre(null)">Tất cả</button>
        ${genres.map(g => `
            <button class="genre-btn" onclick="filterByGenre(${g.id})">${g.name}</button>
        `).join('')}
    `;
}
initGenres();
            ...
            <div class="movie-detail-header">
                ...
                <div class="movie-detail-info">
                    ...
                    <div class="action-buttons">
                        <button class="btn btn-primary" onclick="showBooking(${movieId}, '${movie.title.replace(/'/g, "\\'")}')">🎟️ Đặt vé</button>
                        <button class="btn btn-secondary" onclick="showConcession(${movieId}, '${movie.title.replace(/'/g, "\\'")}')">🍿 Đặt bắp nước</button>
                    </div>
                </div>
            </div>
            ${trailer ? `
...
function showBooking(movieId, movieTitle) {
    ...
    content.innerHTML = `
        ...
        <div class="booking-form">
            ...
            <button class="btn btn-primary" style="width: 100%; margin-top: 1rem;" onclick="confirmBooking('${movieTitle.replace(/'/g, "\\'")}')">Xác nhận đặt vé</button>
        </div>
    `;
    ...
    content.innerHTML = `
        ...
        <div class="concession-form">
            ...
            <div style="margin-top: 2rem; padding: 1.5rem; background: rgba(243, 156, 18, 0.2); border-radius: 10px;">
                ...
                <button class="btn btn-primary" style="width: 100%;" onclick="confirmConcession('${movieTitle.replace(/'/g, "\\'")}')">✅ Xác nhận đặt hàng</button>
            </div>
        </div>
    `;
...
function showVouchers() {
    ...
    //                 <button class="btn btn-primary" onclick="copyVoucher('${v.code}')">📋 Sao chép mã</button>
    //             </div>
    //         `).join('')}
    //     </div>
    //     ${currentUser.role === 'admin' ? `
    //         <div style="margin-top: 2rem; padding: 1.5rem; background: rgba(255, 255, 255, 0.05); border-radius: 10px;">
    //             <button class="btn btn-secondary" onclick="manageVouchers()">⚙️ Quản lý voucher</button>
    //         </div>
    //     ` : ''}
    // `;
    const modal = document.getElementById('movieModal');
                ...
                <div class="voucher-list">
                    ${vouchers.map(v => `
                        <div class="voucher-card">
                            <div class="voucher-code">${v.code}</div>
                            <div>${v.desc}</div>
                            <button class="btn btn-primary" style="margin-top: 1rem;" onclick="copyVoucher('${v.code}')">Sao chép mã</button>
                        </div>
                    `).join('')}
                </div>
            `;
                                    ...
                                    ${currentUser.role === 'admin' ? `
                                        <td>
                                            ${order.status === 'pending' ? `
                                                <button class="btn btn-success" style="padding: 0.5rem 1rem; font-size: 0.85rem; margin-bottom: 0.3rem;" onclick="updateOrderStatus(${order.id}, 'completed')">✅ Hoàn thành</button>
                                                <button class="btn btn-danger" style="padding: 0.5rem 1rem; font-size: 0.85rem;" onclick="updateOrderStatus(${order.id}, 'cancelled')">❌ Hủy</button>
                                            ` : '-'}
                                        </td>
                                    ` : ''}
                                </tr>
    ...
    content.innerHTML = `
        ...
        <div class="admin-dashboard">
            ...
            <div class="admin-section">
                ...
                <div class="action-buttons">
                    <button class="btn btn-primary" onclick="showOrders()">📋 Xem tất cả đơn hàng</button>
                    <button class="btn btn-secondary" onclick="manageVouchers()">🎫 Quản lý voucher</button>
                    <button class="btn btn-secondary" onclick="manageConcessions()">🍿 Quản lý bắp nước</button>
                </div>
            </div>
        </div>
    `;
    ...
    content.innerHTML = `
        ...
        <div class="admin-section">
            <div class="voucher-list">
                ${vouchers.map((v, i) => `
                    <div class="voucher-card">
                        <div class="voucher-code">${v.code}</div>
                        <div style="margin-bottom: 1rem;">${v.desc}</div>
                        <button class="btn btn-danger" onclick="deleteVoucher(${i})">❌ Xóa</button>
                    </div>
                `).join('')}
            </div>
            <div style="margin-top: 2rem; padding: 2rem; background: rgba(233, 69, 96, 0.1); border-radius: 10px;">
                ...
                <button class="btn btn-primary" onclick="addVoucher()">✅ Thêm voucher</button>
            </div>
        </div>
    `;
}
    ...
    content.innerHTML = `
        ...
        <div class="admin-section">
                ...
                <tbody>
                    ${concessions.map((item, i) => `
                        <tr>
                            ...
                            <td>
                                <button class="btn btn-secondary" style="padding: 0.5rem 1rem; font-size: 0.9rem; margin-right: 0.5rem;" onclick="editConcession(${i})">✏️ Sửa</button>
                                <button class="btn btn-danger" style="padding: 0.5rem 1rem; font-size: 0.9rem;" onclick="deleteConcession(${i})">❌ Xóa</button>
                            </td>
                        </tr>
                    `).join('')}
                </tbody>
            ...
            <div style="margin-top: 2rem; padding: 2rem; background: rgba(233, 69, 96, 0.1); border-radius: 10px;">
                ...
                <button class="btn btn-primary" onclick="addConcession()">✅ Thêm món</button>
            </div>
        </div>
    `;
}
